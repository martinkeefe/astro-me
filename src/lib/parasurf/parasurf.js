...
function parasurf(elem_id, shape_name) {
  ...
  var dom = [domain(shape.domain[0], 0, 1), domain(shape.domain[1], 0, 1)];
  var param = [1, 1, 1];
  var t = 1;
  ...
  function setup(canvas, width, height) {
    if (webglAvailable()) {
      ...
      canvas.outerHTML =
        '<p style="color:red; text-align:center">WebGL is not available in this browser.</p>';
      return;
    }
  ...
  function makeScene() {
    ...
    // Lighting
    if (use_normals) {
      body_material = new THREE.MeshNormalMaterial({
      ...
      body_material = new THREE.MeshLambertMaterial({
        side: THREE.DoubleSide,
        color: 0xada7e4, // 10PB 7/8
        polygonOffset: true,
        polygonOffsetFactor: 1,
        polygonOffsetUnits: 1,
      });
  ...
  function XUI(elem_id) {
    ...
    var para_h =
      shape.param && shape.param.length ? (1 + shape.param.length) * chk_h : 0;
    ...
    var gui = new xgui({
      ...
      height: height,
      textColor: "#BBB",
      backgroundColor: "rgb(91,97,111)",
      dimColor: "rgb(124,148,188)",
      frontColor: "rgb(220,200,118)",
    });
    ...
    extras.forEach(function (extra, i) {
      ...
      if (Array.isArray(extra.geom)) {
        extra.geom.forEach(function (g) {
          extra.group.add(
            new THREE.LineSegments(g.clone(), extra_wire_material[i % 3])
          );
        });
      } else {
        extra.group.add(
          new THREE.LineSegments(
            extra.geom.wire.clone(),
            extra_wire_material[i % 3]
          )
        );
      }
